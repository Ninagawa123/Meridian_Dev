...
// Meridian_TWIN_for_Teensy_20221025 By Izumi Ninagawa
// MIT Licenced.
// Meridan TWIN Teensy4.0用スクリプト　20221025版
...
//【更新情報】
// 221025 BNO055に対応（調整の余地あり）★夜に続きの作業をすべき！！！！！！！
...
/* 頻繁に変更するであろう#DEFINE */
#define VERSION "Meridian_TWIN_for_Teensy_2022.10.25" // バージョン表示
...
void setyaw()
{
    if (IMUAHRS_MOUNT == 1) // MPU6050
    {
        ...
        s_spi_meridim.sval[0] = UPDATE_YAW_CENTER + 1;
    }
...
void setup()
{
    ...
    /* I2C接続センサーの設定 */
    if ((IMUAHRS_MOUNT == 1) or (IMUAHRS_MOUNT == 3)) //
    {
        setupIMUAHRS();
    }
...
void loop()
{
    ...
    if (ESP32_MOUNT)
    {
        ...
        if (frame_sync_r_expect == r_spi_meridim.usval[1]) // 受信シーケンシャルカウンタの値が予想通りなら,
        {
