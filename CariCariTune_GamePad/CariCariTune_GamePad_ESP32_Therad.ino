...
UnionData s_upd_meridim; //送信用共用体のインスタンスを宣言
UnionData r_upd_meridim; //受信用共用体のインスタンスを宣言
UnionData r_spi_meridim;
...
void PS4pad_receive() {
  ...
  if (PS4.isConnected()) {
    ...
    s_upd_meridim.sval[80] = pad_btn;
    s_upd_meridim.sval[81] = pad_stick_L_x * 256 + pad_stick_L_y;
    s_upd_meridim.sval[82] = pad_stick_R_x * 256 + pad_stick_R_y;
    s_upd_meridim.sval[83] = pad_L2_val * 256 + pad_R2_val;
...
void loop()
{
  ...
  //---- < 5 > U D P 送 信 ----------------------------------------------
  s_upd_meridim.sval[80] = pad_btn;
  s_upd_meridim.sval[81] = pad_stick_L;
  Serial.println(s_upd_meridim.sval[80], BIN);
  Serial.print((s_upd_meridim.bval[162]));
  Serial.print(",");
  Serial.println((s_upd_meridim.bval[163]));
