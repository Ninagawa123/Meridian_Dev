...
// Meridian_TWIN_for_ESP32_20220716 By Izumi Ninagawa & Meridian Project
// MIT Licenced.
//
...
#define VERSION "Hello, This is Meridian_TWIN_for_ESP32_20220716." // バージョン表示
...
#define SEND_IP "192.168.1.x"          // 送り先のPCのIPアドレス（PCのIPアドレスを調べておく）
...
void setup()
{
  ...
  */
  ...
  /* スレッドの開始 */
  // (ES-4-10) マルチスレッドの宣言（無線系はすべてCORE0で動く.メインループはCORE1）
...
void loop()
{
  ...
  // ★★★★★★ ここは 上の処理が終わったことを確認して次に進むようにしたい。
  // 空っぽのファイルを送ったりするのは無駄かも。
  // 一方で、タイムアウトの概念はあってもよいのかも？

  if (slave.remained() == 0)//★★★★★★ ここの処理はなくても動くかも
  { //キューに入れられた（完了していない）トランザクションのサイズが0なら実行.
    ...
    memcpy(s_udp_meridim.bval, r_spi_meridim_dma, MSG_BUFF);//+4は不要
