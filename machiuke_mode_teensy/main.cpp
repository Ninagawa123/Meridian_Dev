...
// Meridian_TWIN_for_Teensy_20221022 By Izumi Ninagawa
// MIT Licenced.
// Meridan TWIN Teensy4.0用スクリプト　20221022版
...
/* 頻繁に変更するであろう#DEFINE */
#define VERSION "Meridian_TWIN_for_Teensy_2022.10.22" // バージョン表示
...
#define IMUAHRS_MOUNT 3 // IMU/AHRSの搭載状況 0:off, 1:MPU6050(GY-521), 2:MPU9250(GY-6050/GY-9250) 3:BNO055
...
short frame_sync_s = 0;           // フレーム毎に-30000~30000をカウントし、送信用Meridm[1]に格納
short frame_sync_r_expect = 0;    // フレーム毎にに-30000~30000をカウントし、受信値と比較
short frame_sync_r_resv = 0;      // 今フレームに受信したframe_sync_rを格納
short frame_sync_r_last = 0;      // 前回フレームに受信したframe_sync_resvをキープ ★★★
...
void setup()
{
    ...
    {
        setupIMUAHRS(); // MPU6050の初期化
        /* MPU6050の割り込み設定 */
        MsTimer2::set(IMUAHRS_FREQ, IMUAHRS_getYawPitchRoll); // MPUの情報を取得 MUAHRS_FREQ msごとにチェック
        MsTimer2::start();
    }
    else if (IMUAHRS_MOUNT == 3) // BNO055の場合
    {
        ...
        {
            Serial.println("BNO055 mount... OK.");
            delay(50);
...
void loop()
{
    ...
    if (ESP32_MOUNT)
    {
        ...
        frame_sync_r_resv = r_spi_meridim.sval[1]; //
   ...
   //////// < 3.5 > S P I 送 信 の タ イ ミ ン グ を 外 部 に 依 存 す る「待 受 モ ー ド 」の 処 理 ///
    ...
    { // SPI送信のタイミングを外部に依存する「待受モード」の場合, 新しいデータが来るまで以下の処理を飛ばして待受する.
        ...
        // Serial.println(r_spi_meridim.sval[15]); //ボタンデータ表示
