// Meridian_core_for_ESP32_2022.05.06a By Izumi Ninagawa & Meridian Project
// MIT Licenced.
//
...
// (ES-1-1) 変更頻度高め
#define VERSION "Meridian_core_for_ESP32_2022.05.06a"//バージョン表示
...
//#define AP_SSID "merinet2_2G" //アクセスポイントのAP_SSID
//#define AP_PASS "meridim90" //アクセスポイントのパスワード
//#define SEND_IP "192.168.14.2" //送り先のPCのIPアドレス（PCのIPアドレスを調べておく）

//IPを固定する場合は下記の4項目を設定し、(ES-4-1) WiFi.configを有効にする 固定しない場合はコメントアウト必須
//IPAddress ip(192,168,14,10);//ESP32のIPアドレスを固定する場合のアドレス
//IPAddress subnet(255,255,255,0);//ESP32のIPアドレス固定する場合のサブネット
//IPAddress gateway(192,168,14,1);//ルーターのゲートウェイを入れる
...
bool udp_receiving = false;//UDPスレッドでの受信中フラグ（送信抑制）
bool udp_sending = false;//UDPスレッドでの送信中フラグ（受信抑制）
...
void setup()
{
  ...
  */
  ...
  xTaskCreatePinnedToCore(Core0_UDP_s, "Core0_UDP_s", 4096, NULL, 20, &thp[0], 0);
...
void Core0_UDP_r(void *args) {//サブCPU(Core0)で実行するプログラム
  delay(1000);
  while (true) {
    if (udp_sending == false) {
      receiveUDP();//常にUDPの受信を待ちrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr受けし、受信が完了したらフラグを挙げる
...
void Core0_UDP_s(void *args) {//サブCPU(Core0)で実行するプログラム
  ...
  while (true) {
    if (udp_send_flag) {//メインループにより送信フラグが挙がったら送信実行
      ...
      udp_sending = true;//送信busyフラグを挙げる.
      sendUDP();
      delayMicroseconds(10);
      udp_sending = false;//送信busyフラグを下げる.
      udp_send_flag = false;
    }
    delay(1);//1/1000秒待つ
  }
...
void loop()
{
  ...
  if (slave.remained() == 0) {//キューに入れられた（完了していない）トランザクションのサイズが0なら実行.
    ...
    //----  [ 5 ]  U D P 送 信 ----------------------------------------------
    //[5-1] UDP送信を実行
    while (udp_receiving) //UDPが受信処理中なら送信しない
    {
      delayMicroseconds(2);
      //delay(1);
    }
