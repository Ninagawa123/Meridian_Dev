//Meridian_for_ESP32_ControlPad_220110
//ESP32_DevKitC
...
#define SEND_IP "192.168.1.7" //送り先のIPアドレス
...
void loop()
{
  ...
  // SPI受信配列からUDP送信配列にデータを転写
    ...
    if ((short)s_upd_message_buf.sval[MSG_SIZE - 1] == (short)checksum)//チェックがOKならバッファから受信配列に転記
    {
      ...
      s_upd_message_buf.sval[49] = 10000;
    }
    else
    {
      s_upd_message_buf.sval[49] = -10000;//エラーメッセージを何かしら格納
    }
