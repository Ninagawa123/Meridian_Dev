// Meridian_core_for_ESP32_2022.04.11 By Izumi Ninagawa & Meridian Project
// MIT Licenced.
//
// Teensy4.0 - (SPI) - ESP32DevKitC - (Wifi/UDP) - PC/python
// ESP32用のマルチスレッド化したMeiridian_core
// PS4コントローラ再対応
// UDP送受、UDP送信、両方ともスレッド化し、フラグによりデータの流れを円滑化。PC側はPython版のMeridian_consoleがなめらか対応。
// (ただしBTリモコンを接続した場合に処理落ちありで要改善、Unity版と接続した場合もまだ処理落ちあり)

#define VERSION "Meridian_core_for_ESP32_2022.04.11"
...
void setup()
{
  ...
  xTaskCreatePinnedToCore(Core0_UDP_r, "Core0_UDP_r", 4096, NULL, 8, &thp[1], 0);
  xTaskCreatePinnedToCore(Core1_BT_r, "Core1_BT_r", 8192, NULL, 5, &thp[2], 1);
...
void Core1_BT_r(void *args) {//サブCPU(Core0)で実行するプログラム
...
void loop()
{
  ...
  //---- < 1 > U D P 受 信 --------------------------------------------------
  //[1-1] UDP受信の実行（UDP受信用別スレッド）

  //[1-2] UDP受信配列からSPI送信配列にデータを転写
  if (udp_resv_flag) {//UDP受信完了フラグをチェック(フラグが挙がっていれば UDP受信データ「r_udp_meridim」に中身が入った状態)
    udp_resv_flag = 0;//UDP受信完了フラグを下げる
    ...
    if (checksum_rslt(r_udp_meridim.sval, MSG_SIZE))
    {
     ...
     // Serial.println("UDPrvNG***");
      ...
      Serial.println("[1-2]ng");
    }
    //s_spi_meridim.bval[177] |= 0b00010000;//meridimの[88]番の14ビット目(ESPのUPD受信成否)のフラグを上げる//★あとで消すテスト用
  }
  ...
  if (slave.remained() == 0) {//キューに入れられた（完了していない）トランザクションのサイズが0なら実行
    ...
    spi_resv_flag = 1;//SPI受信完了フラグを挙げる
    frame_count = frame_count + 1;//SPIの受信をもってフレームカウントとする
  }
  //udp_resv_process_queue_flag = false;//UDP受信済みデータの処理待ちフラグを下げる
  //}
  ...
  if (spi_resv_flag == 1) {//SPI受信完了フラグが立っていればUDP送信スレッドにフラグで知らせる
    ...
    //Serial.println("[4-1] copied to r_spi_meridim_dma");

    //---- < 5 > U D P 送 信 ----------------------------------------------
    //[5-1] UDP送信を実行（フラグを立ててスレッドに知らせる）

    udp_send_flag = 1;//UDP送信準備完了フラグを挙げる(スレッドCore0_UDP_sで送信実行)
    spi_resv_flag = 0;//SPI受信完了フラグを下げる
    //Serial.println("[5-1] ready to send udp");
