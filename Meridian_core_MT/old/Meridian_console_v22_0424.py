...
#2022.04.23 ROSボタンは機能するがパワーオン時は取扱注意。（ROSの送受信を両方チェックしたりサーボパワーオン組み合わせると現状不具合あり）
#2022.04.24 連番スキップ関連の表示を追加
...
# スキップ検出のカウントアップおよびチェックは動作アルゴリズムがまだまだ怪しい。
...
#定数
TITLE_VERSION="Meridian Console v22.0424" #DPGのウィンドウタイトル兼バージョン表示
        ...
        with closing(sock):
            while True:
                ...
                if checksum[0] == r_meridim[MSG_SIZE-1]:
                    ...
                    if (r_meridim_disp[88] >> 10 & 1) == 1:#エラーフラグ10ビット目（ESPのPCからのUDP受信のフレーム連番スキップフラグ）を調べる
                        error_count_esp_skip += 1
                    if (r_meridim_disp[88] >> 9 & 1) == 1:#エラーフラグ9ビット目（TeensyのESP経由のPCから受信のフレーム連番スキップフラグ）を調べる
                ...
                frame_sync_r_resv = r_meridim_disp[88] & 0b0000000011111111 #カウントを受信
                ...
                #送信用クロックの準備
                frame_sync_s += 1 #送信用のframe_sync_rをカウントアップ
                if frame_sync_s >199:
                    frame_sync_s=0
                temp[0] &= 0b1111111100000000 #下位8ビットをクリア
                temp[0] += frame_sync_s #下位8ビットに受信カウントに+1したものを格納
...
def main():
    ...
    while(True):
        ...
        with dpg.window(label="Command", width=335, height=170,pos=[260,185]):
            ...
            dpg.add_checkbox(label="DemoTest", tag="Action",  callback=set_demo_action, pos=[8,50])
        ...
        while dpg.is_dearpygui_running():
            ...
            if flag_ros1_pub:#ROS送信:joint_statesのpublishを実施
                ...
                #js_meridim.position = \
                #    [math.radians(r_meridim[21]/100*jspn[0]), math.radians(r_meridim[23]/100*jspn[1]), math.radians(r_meridim[25]/100)*jspn[2], math.radians(r_meridim[27]/100*jspn[3]),\
                #        math.radians(r_meridim[29]/100*jspn[4]), math.radians(r_meridim[31]/100*jspn[5]), math.radians(r_meridim[33]/100*jspn[6]), math.radians(r_meridim[35]/100*jspn[7]), \
                #            math.radians(r_meridim[37]/100*jspn[8]),math.radians(r_meridim[39]/100*jspn[9]), math.radians(r_meridim[41]/100*jspn[10]),\
                #                math.radians(r_meridim[51]/100*jspn[15]),math.radians(r_meridim[53]/100*jspn[16]), math.radians(r_meridim[55]/100*jspn[17]), math.radians(r_meridim[57]/100*jspn[18]), \
                #                    math.radians(r_meridim[59]/100*jspn[19]), math.radians(r_meridim[61]/100*jspn[20]), math.radians(r_meridim[63]/100*jspn[21]), math.radians(r_meridim[65]/100*jspn[22]), \
                #                        math.radians(r_meridim[67]/100*jspn[23]),  math.radians(r_meridim[69]/100*jspn[24]), math.radians(r_meridim[71]/100*jspn[25])]
