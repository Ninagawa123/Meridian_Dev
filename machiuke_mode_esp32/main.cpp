// Meridian_TWIN_for_ESP32_20221023 PMバージョン By Izumi Ninagawa & Meridian Project
// MIT Licenced.
//
...
// 221022 TeensyのSPI送信のタイミングを外部に依存する「待受モード」を追加 meridim[0]が5で待ち受けモードフラグ下げ、6で上げ
//        今回の追加箇所に★★★印をつけました

#define VERSION "Meridian_TWIN_for_ESP32_20221023." // バージョン表示
...
//#define SEND_IP "xx.xxx.xxx.xxx"     // 送り先のPCのIPアドレス（PCのIPアドレスを調べておく）
#define BT_MAC_ADDR "00:00:00:00:00:00" // ESP32自身のBluetoothMACアドレス（本プログラムを実行しシリアルモニタで確認）
...
#define SERIAL_PC 115200        // ESP-PC間のシリアル速度（モニタリング表示用）
...
short frame_sync_spi_r_resv = 0;        // 今フレームに受信したframe_sync_rを格納 ★★★
short frame_sync_spi_r_last = 0;        // 前回フレームに受信したframe_sync_resvをキープ ★★★

/* エラーカウント */ //★★★
int spi_ok = 0;      // SPI受信のエラーカウント ★★★
int spi_trial = 0;   // SPI受信の試行回数カウント ★★★
...
void loop()
{
  ...
  {
    memcpy(r_spi_meridim.bval, r_spi_meridim_dma, MSG_BUFF + 4); // DMAのSPI受信データをSPI受信配列に転記 ★★★
    slave.pop();                                                 // DMAのデータ配列の先頭を削除

    //------------------------------------------------------------------------
    // [ 1.5 ]  S P I 受 信 デ ー タ の チェックサム と 待受モード 対 応
    //------------------------------------------------------------------------
    // チェックサムがOKならSPI受信配列からUDP送信配列に転記 ★★★
    if (checksum_rslt(r_spi_meridim.sval, MSG_SIZE))
    {
      for (int i = 0; i < MSG_SIZE; i++)
      {
        s_udp_meridim.sval[i] = r_spi_meridim.sval[i];
      }

      spi_ok++;
      s_udp_meridim.bval[MSG_ERR_u] &= B11101111; //エラーフラグ12番(ESPのTeensyからのSPI受信エラー検出)をオフ

      // SPI送信タイミング「待受モード」のコマンドチェック. ★★★
      //if (s_udp_meridim.sval[0] == 5) //　5ならフラグ下げ. Teensy主導
      //{
      //  flag_spi_initiative_outer = false;
      //}
      //else if (s_udp_meridim.sval[0] == 6) // 6 ならフラグ上げ. 外部主導
      //{
    ...
    {
      s_udp_meridim.bval[MSG_ERR_u] |= B00010000; //エラーフラグ12番(ESPのTeensyからのSPI受信エラー検出)をオン
    }

    // @[1-3-1b] シーケンシャルカウンタのデータ格納 ★★★
    frame_sync_spi_r_resv = s_udp_meridim.sval[1]; // ★★★
    ...
    // 待受モード対応. 待受モードかつシーケンシャルカウンタが上がっていない時はUDP送信をキャンセル
    if (frame_sync_spi_r_resv != frame_sync_spi_r_last) //★★★ シーケンシャルカウンタを比較
    {
    ...
    frame_sync_spi_r_last = frame_sync_spi_r_resv; //前回受信したシーケンシャル番号をキープ
