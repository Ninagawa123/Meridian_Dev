...
// Meridian_TWIN_for_Teensy_20220720 By Izumi Ninagawa & Meridian Project
// MIT Licenced.
//
// Meridan TWIN Teensy4.0用スケッチ　20220720版
...
void loop() {
  ...
  if (ESP32_MOUNT) {
    ...
    if ((r_spi_meridim.bval[MSG_ERR_u] >> 7) & B00000001)//Meridim[88] bit15:PCのESP32からのUDP受信エラー
    {
      err_esp_pc ++;
    }
    if ((r_spi_meridim.bval[MSG_ERR_u] >> 6) & B00000001)//Meridim[88] bit14:ESP32のPCからのUDP受信エラー
    {
      err_pc_esp ++;
    }
    if ((r_spi_meridim.bval[MSG_ERR_u] >> 5) & B00000001)//Meridim[88] bit13:TeensyのESPからのSPI受信エラー
    {
      err_esp_tsy ++;
    }
    if ((r_spi_meridim.bval[MSG_ERR_u] >> 4) & B00000001)//Meridim[88] bit12:ESP32のTeensyからのSPI受信エラー
    {
      err_tsy_esp ++;
    }
    if ((r_spi_meridim.bval[MSG_ERR_u] >> 2) & B00000001)//Meridim[88] bit10:UDP→ESP受信のカウントのスキップ
    {
      err_esp_skip ++;
    }
    if ((r_spi_meridim.bval[MSG_ERR_u] >> 1) & B00000001)//Meridim[88] bit9:ESP→Teensy受信のカウントのスキップ
    {
      err_tsy_skip ++;
    }
    if ((r_spi_meridim.bval[MSG_ERR_u]) & B00000001)//Meridim[88] bit8:PC受信のカウントのスキップ
    {
      err_pc_skip ++;
    }
