...
// Meridian_TWIN_for_Teensy_20220717a By Izumi Ninagawa & Meridian Project
// MIT Licenced.
...
// (TS-1-1) 変更頻度高め
#define VERSION "Meridian_TWIN_for_Teensy_2022071" // バージョン表示
...
#define IMU_MOUNT 2           // IMUの搭載状況 0:なし, 1:BNO055, 2:MPU6050(未実装), 3:MPU9250(GY-6050/GY-9250), (※1,2のみ実装済, MeridianBoardではI2Cに接続)
...
bool flag_servo_delay = false;        // サーボのディレイ発生フラグ
bool flag_system_delay = false;       // フレーム終端のディレイ発生フラグ
...
short frame_sync_r_expect = -30000;   // フレーム毎に-30000 ~ 29999をカウントし、受信値と比較
...
unsigned short pad_btn_accum = 0;     // 他デバイスとのボタン押下データ累積
...
void imu_moving_average() {//IMUデータの移動平均値の取得
  ...
  mpu6050_getYawPitchRoll();//IMUから最新データを取得
...
void setyawcenter()
{
  if (IMU_MOUNT == 1) { //BNO055の場合
  }
  //yaw_center = bno055_read[14];

  if (IMU_MOUNT == 2) { //MPU6050の場合
    ...
    if (joypad_frame_count >= JOYPAD_FRAME) {
      ...
      if (buttonData != KRR_BUTTON_FALSE) //ボタンデータが受信できていたら
      {
        ...
        if (monitor_joypad) {
          ...
          pad_btn = 0;
          if ((button_1 & 15) == 15) {//左側十字ボタン全部押しならstart押下とみなす
            pad_btn |= 1;
          }
          else {
            //左側の十時ボタン
          ...
          if ((button_1 & 368) == 368) pad_btn |= 8;//右側十字ボタン全部押しならselect押下とみなす
          else {
            //右側十時ボタン
...
void setup() {
  ...
  // (TS-11-9) 割り込み処理系のセット
  // IMUのデータ取得
  MsTimer2::set(10, mpu6050_getYawPitchRoll); // MPUの情報を取得 10msごとにチェック
  MsTimer2::start();
...
void loop() {
  ...
  if (ESP32_MOUNT) {
    ...
    if (frame_sync_r_expect > 29999) { //予想値が200以上ならカウントを0に戻す
      frame_sync_r_expect = -30000;
    }

    //frame_sync_rの確認(受信フレームにスキップが生じていないかをMeridim[88]の下位8ビットのカウンターで判断)
    frame_sync_r_resv = r_spi_meridim.sval[1];//数値を受け取る
    ...
    if (flag_system_delay) //ディレイ発生フラグが立っていたらエラーフラグ3番(Delay検出)をオン
    {
      r_spi_meridim.bval[177] |= B00000010; //エラーフラグ3番(Delay検出)をオン
      err_delay ++;
    } else
    {
      r_spi_meridim.bval[177] &= B11111011; //エラーフラグ3番(Delay検出)をオフ
    }
    ...
    // [2-3] 全経路のエラー数の表示
    /*
      Serial.print("resv-seq:");
      Serial.print(int(frame_sync_r_resv));//
      Serial.print(",");
      Serial.print("expt-seq:");
      Serial.print(int(frame_sync_r_expect));//
      Serial.print(",");
      Serial.print("tsy-skip:");
      Serial.print(((r_spi_meridim.bval[177] >> 1) & B00000001)*200);//
      Serial.print(",");
      Serial.print("delay:");
      Serial.print(((r_spi_meridim.bval[177] >> 3) & B00000001)*200);//
      Serial.println();
    */
  }
  ...
  if (JOYPAD_MOUNT == 2) {//KRC-5FH+KRR-5FHが接続設定されていれば受信チェック
    joypad_read();
    pad_btn_accum = r_spi_meridim.sval[15] | pad_btn;
    Serial.println(pad_btn_accum);
  }
  ...
  s_spi_meridim.sval[15] = pad_btn_accum;
  //s_spi_meridim.sval[16] = 未実装;
  //s_spi_meridim.sval[17] = 未実装;
  //s_spi_meridim.sval[18] = 未実装;
    
  // [9-5] サーボIDごとにの現在位置もしくは計算結果を配列に格納
  for (int i = 0; i < 15; i++) {
  ...
  if (curr > merc) { // 現在時刻がフレーム管理時計を超えていたらアラートを出す
    ...
    flag_system_delay = true;
  }
  else {
    digitalWrite(ERR_LED, LOW);//処理が収まっていればLEDを消灯
    flag_system_delay = false;
  }
