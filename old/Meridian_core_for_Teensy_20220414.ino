...
// Meridian_core_for_Teensy_2022.04.14 By Izumi Ninagawa & Meridian Project
// MIT Licenced.

//This code is for Teensy 4.0
//PC計測200Hz以上での安定動作を確認→嘘でした。いいとこ100Hz,50で超安定。
//
//MeridianControlPanelDPG対応
//マスターコマンド改訂 (S-3-3)
//KRC-5FH改訂(S-3-5)

#define VERSION "Meridian_core_MT_for_Teensy_2022.04.14"
...
int err_esp_pc = 0;//テスト用
int err_pc_esp = 0;//テスト用
int err_esp_tsy = 0;//テスト用
int err_tsy_esp = 0;//テスト用
int err_tsy_skip = 0;//テスト用
int err_pc_skip = 0;//テスト用
...
void loop() {
  ...
  //Meridianに格納するframe_clockのカウントアップ処理
  frame_clock ++;
  if (frame_clock > 199) {
    frame_clock = 0;
  }
  ...
  //送信frame_clockの格納
  if (frame_clock>198){
    s_spi_meridim.bval[176] = 0;
  }else
  {
    s_spi_meridim.bval[176] = frame_clock+1;
  }
  ...
  if (ESP32_MOUNT) {
    ...
    /*
      Serial.print(checksum_val(r_spi_meridim_dma.sval, MSG_SIZE));
      Serial.print("/");
      Serial.println(r_spi_meridim_dma.sval[MSG_SIZE-1]);
    */
    ...
    frame_clock_resv = r_spi_meridim.bval[176];
    //if (frame_clock_resv - frame_clock_resv_past == 1)//frame_clockの前回との差が1以外なら受信スキップあり
    if (frame_clock_resv + 1 == frame_clock) //frame_clockの前回との差が1以外なら受信スキップあり
    {
      r_spi_meridim.bval[177] &= B11111101; //エラーフラグ9番(Teensy受信のスキップ検出)をオフ
      //frame_clock_resv_past = frame_clock_resv;//次フレーム用に今回の受信frame_clockをキープ
    } else if ((frame_clock_resv == 199) and (frame_clock == 0))//frame_clockを0に戻すタイミングの場合
    {
      r_spi_meridim.bval[177] &= B11111101; //エラーフラグ9番(Teensy受信のスキップ検出)をオフ
      //frame_clock_resv_past = frame_clock_resv;//次フレーム用に今回の受信frame_clockをキープ
    } else
    {
      ...
      err_tsy_skip ++;
      //frame_clock_resv_past = frame_clock_resv + 1; //次フレーム用に今回の受信frame_clockをキープだが、取りこぼし分の1を追加
    }
    ...
    if (monitor_resv_error) {//受信エラー率の表示
      if (spi_trial % 200 == 0) {
    ...
    if (monitor_all_error) {//全経路のエラー数の表示
