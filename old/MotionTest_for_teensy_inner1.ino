...
//MotionTest_for_teensy_inner1_2022.2.11
//Teensy+MeridianBoard+KHRでの動作テストです。
//Meridianの通信を使用せず、Teensyのみでモーションを生成、実機再生します。
//時折、詰まったような動きになります（断線かも？）
//12V電源では動きますが、5V電源ではトルクが足りず屈伸時にへなります。
//699行目のブロックを書き換えてモーションを変更できます。
...
int frame_count_diff = 4;//サインカーブ動作などのフレームカウントをいくつずつ進めるか
...
void setup() {
  ...
  //サーボ電源オン
  for (int i = 0; i < 12; i ++) {//接続したサーボの数だけ繰り返す。最大は15
    r_spi_meridim.sval[(i * 2) + 20] = 1;
    r_spi_meridim.sval[(i * 2) + 50] = 1;
  }

  //サーボ角度ゼロ
  for (int i = 0; i < 12; i ++) {//接続したサーボの数だけ繰り返す。最大は15
    r_spi_meridim.sval[(i * 2) + 21] = 0;
    r_spi_meridim.sval[(i * 2) + 51] = 0;
  }
...
void loop() {
  ...
  */
  ...
  //モーションの元となるサインカーブ、コサインカーブ
  n = -sin(PI / 2 / 300 * frame_count) * 1000;
  n2 = cos(PI / 2 / 300 * frame_count) * 1000;
  
  //サインカーブ値とフレームカウントのモニタリング
  Serial.print(n);
  Serial.print(",");
  Serial.println(frame_count);

  //サインカーブを元にモーションを生成（自由にアレンジしてください）
  for (int i = 0; i < 12; i ++) {//接続したサーボの数だけ繰り返す。最大は15
    r_spi_meridim.sval[21] = 0;//頭ヨー
    r_spi_meridim.sval[23] = 0;//左肩ピッチ
    r_spi_meridim.sval[25] = int(n2) + 1200; //左肩ロール
    r_spi_meridim.sval[27] = int(n) + 800; //左肘ヨー
    r_spi_meridim.sval[29] = int(n);//左肘ピッチ
    r_spi_meridim.sval[31] = 0;//左股ヨー
    r_spi_meridim.sval[33] = 0;//左股ロール
    r_spi_meridim.sval[35] = int(n) * 2 - 3400; //左股ピッチ
    r_spi_meridim.sval[37] = -int(n) * 4 + 6000; //左膝ピッチ
    r_spi_meridim.sval[39] = int(n) * 2 - 3000; //左足首ピッチ
    r_spi_meridim.sval[41] = 0;//左足首ロール

    r_spi_meridim.sval[51] = 0;//腰ヨー
    r_spi_meridim.sval[53] = 0;//右肩ピッチ
    r_spi_meridim.sval[55] = int(n2) + 1200; //右肩ロール
    r_spi_meridim.sval[57] = int(n) + 800; //右肘ヨー
    r_spi_meridim.sval[59] = int(n);//右肘ピッチ
    r_spi_meridim.sval[51] = 0;//右股ヨー
    r_spi_meridim.sval[53] = 0;//右股ロール
    r_spi_meridim.sval[55] = int(n) * 2 - 3400; //右股ピッチ
    r_spi_meridim.sval[57] = -int(n) * 4 + 6000; //右膝ピッチ
    r_spi_meridim.sval[59] = int(n) * 2 - 3000; //右足首ピッチ
    r_spi_meridim.sval[61] = 0;//右足首ロール
  }

  //meridimのdegreeをサーボに書き込む
  for (int i = 0; i < 15; i++) {
    s_KRS_servo_pos_L[i] = Deg2Krs(float(r_spi_meridim.sval[i * 2 + 21]) / 100 * idl_pn[i], idl_n[i]); //
    s_KRS_servo_pos_R[i] = Deg2Krs(float(r_spi_meridim.sval[i * 2 + 21]) / 100 * idl_pn[i], idl_n[i]); //
  }
